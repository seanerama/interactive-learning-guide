...
/**
 * interactive-learning-guide installer — Multi-runtime installation for Claude Code, OpenCode, Gemini CLI, and Codex
 *
 * Usage: npx interactive-learning-guide [--claude|--opencode|--gemini|--codex|--all] [-g|--global] [-l|--local] [-u|--uninstall]
 */
...
const banner = '\n' +
  ...
  '  ██║███████╗╚██████╔╝\n' +
  '  ╚═╝╚══════╝ ╚═════╝ ' + reset + '\n\n' +
  '  ' + bold + 'Interactive Learning Guide' + reset + ' ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Transform books and technical content into\n' +
  '  interactive HTML learning guides\n';
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx interactive-learning-guide [options]\n`);
  ...
  console.log(`    ${dim}npx interactive-learning-guide --claude --global${reset}    Install for Claude Code`);
  console.log(`    ${dim}npx interactive-learning-guide --all --global${reset}       Install for all runtimes`);
  console.log(`    ${dim}npx interactive-learning-guide --claude --uninstall${reset} Remove from Claude Code\n`);
  process.exit(0);
}
...
function convertForOpencode(content, configDir) {
  ...
  converted = converted.replace(/\/ilg:/g, '/ilg-');
...
function convertForCodex(content, configDir) {
  ...
  converted = converted.replace(/~\/\.claude\//g, configDir.replace(os.homedir(), '~/') + '/');
  converted = converted.replace(/\/ilg:/g, '/ilg-');
  converted = converted.replace(/\$ARGUMENTS/g, '{{ILG_ARGS}}');
...
function installForRuntime(runtime) {
  ...
  // 1. Copy commands
  const srcCommands = path.join(__dirname, '..', 'commands', 'ilg');
  let destCommands;

  if (runtime === 'claude') {
    destCommands = path.join(configDir, 'commands', 'ilg');
  } else if (runtime === 'opencode') {
    destCommands = path.join(configDir, 'custom-instructions', 'ilg');
  } else if (runtime === 'gemini') {
    destCommands = path.join(configDir, 'commands', 'ilg');
  } else {
    destCommands = path.join(configDir, 'skills', 'ilg');
  }
  ...
  // 2. Copy internals (ilg/)
  const srcInternals = path.join(__dirname, '..', 'ilg');
  const destInternals = path.join(configDir, 'ilg');
  copyDirRecursive(srcInternals, destInternals);
  console.log(`  ${green}✓${reset} Workflows and templates installed`);

  // 3. Install hooks (Claude Code only)
  if (runtime === 'claude') {
    const hooksDistDir = path.join(__dirname, '..', 'hooks', 'dist');
    const hooksSrcDir = path.join(__dirname, '..', 'hooks', 'src');
    const distHasHooks = fs.existsSync(hooksDistDir) && fs.readdirSync(hooksDistDir).some(f => f.startsWith('ilg-'));
    const hooksSource = distHasHooks ? hooksDistDir : hooksSrcDir;

    if (fs.existsSync(hooksSource)) {
      const hookFiles = fs.readdirSync(hooksSource).filter(f => f.startsWith('ilg-'));
      ...
      if (hookFiles.length > 0) {
        ...
        // Clean up previous ILG hooks
        for (const event of ['PostToolUse', 'SessionStart']) {
          if (Array.isArray(settings.hooks[event])) {
            settings.hooks[event] = settings.hooks[event].filter(h =>
              !(h.command && h.command.includes('ilg-')) &&
              !(h.hooks && h.hooks.some(hh => hh.command && hh.command.includes('ilg-')))
            );
        ...
        const hookConfig = [
          {
            event: 'SessionStart',
            matcher: '',
            file: 'ilg-session-start.js',
          },
        ];

        for (const config of hookConfig) {
          if (!hookFiles.includes(config.file)) continue;
...
function uninstallForRuntime(runtime) {
  ...
  // Remove commands
  let commandsDir;
  if (runtime === 'claude') commandsDir = path.join(configDir, 'commands', 'ilg');
  else if (runtime === 'opencode') commandsDir = path.join(configDir, 'custom-instructions', 'ilg');
  else if (runtime === 'gemini') commandsDir = path.join(configDir, 'commands', 'ilg');
  else commandsDir = path.join(configDir, 'skills', 'ilg');

  removeDir(commandsDir);

  // Remove internals
  removeDir(path.join(configDir, 'ilg'));
  ...
  if (runtime === 'claude') {
    ...
    if (settings.hooks) {
      for (const event of Object.keys(settings.hooks)) {
        ...
        settings.hooks[event] = settings.hooks[event].filter(h => {
          if (h.command && h.command.includes('ilg-')) return false;
          if (h.hooks && h.hooks.some(hh => hh.command && hh.command.includes('ilg-'))) return false;
          return true;
        });
        if (settings.hooks[event].length === 0) delete settings.hooks[event];
      }
    ...
    if (fs.existsSync(hooksDir)) {
      const hookFiles = fs.readdirSync(hooksDir).filter(f => f.startsWith('ilg-'));
      for (const file of hookFiles) {
...
async function main() {
  ...
  if (!hasUninstall) {
    ...
    console.log(`\n  ${dim}Start a new session and run /ilg:help to see available commands.${reset}\n`);
  } else {
